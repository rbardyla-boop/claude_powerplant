...
interface BrokerState {
  ...
  // Action counters for run classification
  readCount: number
  writeCount: number
  checkCount: number
...
async function handleRunCheck(state: BrokerState, input: unknown): Promise<string> {
  ...
  // Infer check kind for diagnostic extraction.
...
export async function runProjectPilotBrokerSession(opts: {
  ...
  while (true) {
    if (state.totalCustomToolCalls >= SPRINT4A_MAX_TOOL_CALLS) {
      // Budget exhausted — classify and break cleanly.
      // The run is ineligible; classification is written to the artifact dir below.
      state.budgetExhausted = true
      console.warn(
        `[broker] safety: ${SPRINT4A_MAX_TOOL_CALLS} custom tool calls reached — stopping session`,
      )
      break
    }
  ...
  // ── Run classification ────────────────────────────────────────────────────
  const anyCheckStreak = Object.values(state.checkFailStreaks).some(n => n >= 3)

  type RunTerminationReason = import('../contracts/project-tool-contracts.js').RunTerminationReason
  let terminationReason: RunTerminationReason
  if (state.budgetExhausted) {
    terminationReason = 'FAILED_TOOL_BUDGET_EXHAUSTED'
  } else if (!state.finalizeReceived) {
    terminationReason = 'FAILED_INCOMPLETE_AGENT_RUN'
  } else {
    terminationReason = 'COMPLETED'
  }

  const patchEligibleForApplication = passed && terminationReason === 'COMPLETED'

  const classification: RunClassification = {
    terminationReason,
    patchEligibleForApplication,
    readCount: state.readCount,
    writeCount: state.writeCount,
    checkCount: state.checkCount,
    finalizeAttempted: state.finalizeAttempted,
    artifactsComplete: state.patchPackage !== null,
    repeatedCheckFailures: anyCheckStreak,
    ...(state.lastFailedDiagnostic !== null && !passed
      ? { lastFailedDiagnostic: state.lastFailedDiagnostic }
      : {}),
  }

  // Persist classification so incomplete/budget-exhausted runs leave an honest record.
  try {
    const classificationPath = path.join(state.patchDir, 'RUN_CLASSIFICATION.json')
    fs.writeFileSync(classificationPath, JSON.stringify(classification, null, 2), 'utf-8')
  } catch {
    // Best-effort — never block the return on filesystem write failure.
  }
