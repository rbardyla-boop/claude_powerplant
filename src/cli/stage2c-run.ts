// src/cli/stage2c-run.ts — Stage 2C runner CLI entry point (Steps 1–5).
//
// Usage: node --import=tsx src/cli/stage2c-run.ts --task "..." [--dry-run] [--fake-agent] [--oracle]
//
// TRUST BOUNDARY:
...
function parseArgs(argv: string[]): { task: string; dryRun: boolean; fakeAgent: boolean; oracle: boolean } {
  let task = ''
  let dryRun = false
  let fakeAgent = false
  let oracle = false
  ...
  return { task, dryRun, fakeAgent, oracle }
}
...
if (_isMain) {
  const { task, dryRun, fakeAgent, oracle } = parseArgs(process.argv.slice(2))
  ...
  const result = runStage2cSkeleton({ task, dryRun, fakeAgent, oracle })
