...
- Copy regular file content through an opened source handle, not via a path re-open after lstat:
  - Open with `O_NOFOLLOW` where supported (Linux/macOS). ELOOP on the final symlink component.
  - `fstat` the opened handle (not the path). Verify: regular file, inode + device match walk record, nlink ≤ 1, mtime matches walk record.
  - Create destination exclusively with `O_CREAT | O_EXCL`. Never overwrite an existing snapshot file.
  - Copy bytes from opened handle to destination handle.
  - `fstat` opened source handle again after copy. Fail if size, mtime, or ctime changed.
  - On any copy failure: unlink partial destination, write Gate 0 rejection audit event.
- Intermediate-directory substitution is a residual risk not portably eliminable at the Node.js level. Mitigated by operator-only import policy and `0o700` quarantine permissions.
- Reject any source package containing `.powerplant-meta.json` — this filename is reserved for Powerplant-owned sidecar metadata and must never appear in imported content.
- Only after all entries pass: copy into `candidates/<uuid>/`.
- Powerplant-owned metadata (normalized manifest) is written to `candidates/<uuid>/.powerplant-meta.json` — never to the imported content area.
- The snapshot is Powerplant-controlled and source-detached. True content identity is established at Gate 2 via canonical hash.
...
**Payload boundary rule (required before Phase 1B implementation):**

The canonical content hash identifies the imported skill payload only. It must never include Powerplant-generated sidecar files or evaluation artifacts.

```
Hashed payload:
  SKILL.md
  imported manifest.json (when present in source)
  scripts/*, tests/*, resources/* (when present in source)
  Any other regular file copied from the source package

Explicitly excluded from hash:
  .powerplant-meta.json         (Powerplant-owned sidecar — never in source)
  skill-audit.jsonl             (audit trail — Powerplant-owned)
  skill-registry.json           (registry — Powerplant-owned)
  Evaluation output / reports   (generated by Powerplant)
```

Gate 0 already rejects any source package that contains `.powerplant-meta.json`. This rule ensures the hash boundary is unambiguous: hash what was imported, nothing else.

Implementation:
- Compute SHA-256 over the canonical representation: sorted relative paths (excluding `.powerplant-meta.json`) + file contents.
- Write this hash to `.powerplant-meta.json`'s `sha256` field. Powerplant owns this value.
- An imported package must not contain a pre-populated `sha256` in its manifest (Gate 1 already rejects non-null `sha256` in imported manifests).
